...
from brd_knowledge.core.exceptions import GenerationError, GenerationProviderError
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Answer one question using dense retrieval and Gemini."
    )
...
def main() -> None:
    ...
    configuration = GeminiConfiguration.model_validate(
        {
    ...
    if args.max_output_tokens > configuration.max_output_tokens:
        raise ValueError("max-output-tokens exceeds the Gemini model output limit.")
    ...
    provider = GeminiLLMProvider(settings.gemini_api_key, configuration)
    try:
